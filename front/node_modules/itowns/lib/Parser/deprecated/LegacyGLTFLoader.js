    ...
    return this._withDependencies(["shaders", "textures"]).then(function (dependencies) {
      return _each(json.materials, function (material) {
        ...
        if (khr_material) {
          ...
          var technique = json.techniques[material.technique]; // If shaders are Cesium specific, set up a MeshBasicMaterial to avoid an error at shader compilation

          if (areShadersCesiumSpecific(dependencies.shaders)) {
            materialType = THREE.MeshBasicMaterial; // Retrieve texture from uniforms so it is not lost in the process.
            ...
            if (program) {
              ...
              var _uniforms = technique.uniforms;

              for (var _uniformId in _uniforms) {
                var _pname = _uniforms[_uniformId];
                var _shaderParam = technique.parameters[_pname];
                var _ptype = _shaderParam.type;

                if (WEBGL_TYPE[_ptype]) {
                  var pcount = _shaderParam.count;

                  var _value = void 0;

                  if (material.values !== undefined) _value = material.values[_pname];
                  var uvalue = new WEBGL_TYPE[_ptype]();
                  var usemantic = _shaderParam.semantic;
                  var unode = _shaderParam.node;

                  switch (_ptype) {
                    case WEBGL_CONSTANTS.FLOAT:
                      uvalue = _shaderParam.value;

                      if (_pname == "transparency") {
                        materialParams.transparent = true;
                      }
                    ...
                    case WEBGL_CONSTANTS.FLOAT_MAT3:
                      if (_shaderParam && _shaderParam.value) {
                        uvalue.fromArray(_shaderParam.value);
                      }

                      if (_value) {
                        uvalue.fromArray(_value);
                      }
                    ...
                    case WEBGL_CONSTANTS.FLOAT_MAT4:
                      if (pcount) {
                        ...
                        for (var mi = 0; mi < pcount; mi++) {
                          uvalue[mi] = new WEBGL_TYPE[_ptype]();
                        }

                        if (_shaderParam && _shaderParam.value) {
                          var m4v = _shaderParam.value;
                    ...
                    case WEBGL_CONSTANTS.SAMPLER_2D:
                      if (_value !== undefined) {
                        uvalue = dependencies.textures[_value];
                      } else if (_shaderParam.value !== undefined) {
                        uvalue = dependencies.textures[_shaderParam.value];
                      } else {
                        uvalue = null;
                      }
                  ...
                  materialParams.uniforms[_uniformId] = {
                    value: uvalue,
                    semantic: usemantic,
                    node: unode
                  };
                } else {
                  throw new Error("Unknown shader uniform param type: " + _ptype);
                }
              }
